...
// Helper to recursively find a menu item
const findMenuItem = (items: MenuItem[], id: number): MenuItem | undefined => {
...
const ReportsPage: React.FC = () => {
  ...
  const [currentSubMenus, setCurrentSubMenus] = useState<MenuItem[]>([]);
      ...
      if (menu) {
        ...
        if (!isNaN(menuId)) {
          ...
          if (reportsResponse.success && reportsResponse.data) {
            ...
            const selectedMenu = findMenuItem(menuResponse, menuId);
            setSelectedSection(selectedMenu?.name || "");
            setCurrentSubMenus(selectedMenu?.children || []);
          }
        }
      } else {
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 flex">
      ...
      <div className={`flex-1 flex flex-col transition-all duration-300 ${sidebarCollapsed ? 'lg:ml-16' : 'lg:ml-64'}`}>
        ...
        <main className="flex-1 p-8">
          {menu && (reports.length > 0 || currentSubMenus.length > 0) ? (
            // Show specific section reports and sub-categories
            <div>
              <div className="mb-6">
                <h2 className="text-xl font-semibold text-gray-900 mb-2">
                  {selectedSection || "Available Reports"}
                </h2>
                <p className="text-gray-600">
                  {reports.length > 0
                    ? "Click on any report to open it in a new tab."
                    : "Select a sub-category to view reports."}
                </p>
              </div>
              ...
              {currentSubMenus.length > 0 && (
                <div className="mb-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {currentSubMenus.map(renderSectionCard)}
                  </div>
                </div>
              )}
